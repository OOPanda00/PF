...
@Composable fun PandaFinanceApp(vm: FinanceViewModel = viewModel()) {
    ...
    CompositionLocalProvider(LocalAppLanguage provides language) {
        CompositionLocalProvider(androidx.compose.ui.platform.LocalLayoutDirection provides if (language == "en") androidx.compose.ui.unit.LayoutDirection.Ltr else androidx.compose.ui.unit.LayoutDirection.Rtl) {
            MaterialTheme(colorScheme = if (darkMode) darkColorScheme() else lightColorScheme()) { Surface(Modifier.fillMaxSize()) { FinanceScreen(vm, reserve, darkMode, language, appLock, biometricEnabled, biometricAvailable) } }
        }
    }
}
...
@Composable private fun SettleDialog(a:Account,suggested:Long,suggestedPercent:Int,balance:Long,dismiss:()->Unit,save:(Long)->Unit){var p by remember{mutableFloatStateOf(if(a.remaining>0)suggested*100f/a.remaining else 0f)};var amount by remember{mutableStateOf(suggested.toString())};val max=if(a.type==AccountType.OBLIGATION)minOf(a.remaining,balance.coerceAtLeast(0)) else a.remaining;AlertDialog(onDismissRequest=dismiss,title={Text(if(a.type==AccountType.OBLIGATION)tr("تسجيل سداد","Record payment") else tr("تسجيل استلام","Record receipt"))},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)){Text("${a.name} • المتبقي ${money(a.remaining)}",fontWeight=FontWeight.Bold);Text(tr("المقترح: ${money(suggested)} ($suggestedPercent%)","Suggested: ${money(suggested)} ($suggestedPercent%)"));Text(tr("النسبة: ${p.toInt()}%","Percentage: ${p.toInt()}%"));Slider(value=p,onValueChange={p=it;amount=(a.remaining*it/100f).toLong().coerceAtMost(max).toString()},valueRange=0f..100f);AmountField(amount){amount=it.filter(Char::isDigit);p=((amount.toLongOrNull()?:0)*100f/a.remaining.coerceAtLeast(1)).coerceIn(0f,100f)};Text(tr("بعد العملية سيصبح المتبقي: ${money((a.remaining-(amount.toLongOrNull()?:0)).coerceAtLeast(0))}","Remaining after transaction: ${money((a.remaining-(amount.toLongOrNull()?:0)).coerceAtLeast(0))}"))}},confirmButton={TextButton(enabled=(amount.toLongOrNull()?:0) in 1..max,onClick={save(amount.toLong())}){Text(tr("تأكيد","Confirm"))}},dismissButton={TextButton(dismiss){Text(tr("إلغاء","Cancel"))}})}
...
@Composable private fun SettingsDialog(current:Int,dark:Boolean,language:String,lock:Boolean,hasPin:Boolean,biometricEnabled:Boolean,biometricAvailable:Boolean,dismiss:()->Unit,save:(Int,Boolean,String,Boolean,Boolean)->Unit,export:()->Unit,import:()->Unit,setPin:(String)->Unit){
    ...
    AlertDialog(onDismissRequest=dismiss,title={Text(tr("الإعدادات","Settings"))},text={Column(verticalArrangement=Arrangement.spacedBy(10.dp)){
        Text(tr("احتياطي الرصيد: ${v.toInt()}%","Balance reserve: ${v.toInt()}%")); Slider(value=v,onValueChange={v=it},valueRange=0f..80f,steps=15)
        ...
        HorizontalDivider(); Button(export,Modifier.fillMaxWidth()){Text(tr("نسخة احتياطية مشفرة","Encrypted backup"))}; OutlinedButton(import,Modifier.fillMaxWidth()){Text(tr("استعادة نسخة احتياطية","Restore backup"))}
